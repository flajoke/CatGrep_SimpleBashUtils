...
testing()
{
    ...
    valgrind -s --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=valgrind.out ./s21_grep $t
    leak=$(grep valgrind.out -e "ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)")
    (( COUNTER++ ))
